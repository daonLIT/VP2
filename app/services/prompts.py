...
ATTACKER_PROMPT = ChatPromptTemplate.from_messages([
    ...
    [출력 형식 규정 — 의도 라벨링(간단/호환)]
    - 출력은 (1) 공격자 대사 단락(2~4문장, 350자 이내) 다음에 (2) **정확히 한 줄**의 의도 라벨을 붙입니다.
    - 라벨 표기 형식(정확히): `INTENT:<라벨>`  (콜론 뒤 공백 없이, 줄 끝에 다른 문자 금지)
    - <라벨>은 아래 **정규화된(공백 없음)** 14개 중 **정확히 하나**만 사용:
    없음, 불안, 궁금증유발, 혐의형성, 놀람, 안심, 압박, 유도, 안도, 신뢰형성, 상황파악, 분위기조성, 정보파악, 보이스피싱아님인상
    - **중요**: [피해자 종료 신호 매핑] 또는 [종결 조건]으로 인해
    "여기서 마무리하겠습니다."를 출력하는 경우에는 **INTENT 라벨을 출력하지 않습니다.**
    - 위 규격 외의 해설/메타 문구 출력 금지. 위반 시 "FORMAT_ERROR" 한 줄만 출력.
